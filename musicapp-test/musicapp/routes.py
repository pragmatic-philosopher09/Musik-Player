...
from musicapp.forms import UploadPodcastForm, UploadPostForm, UploadSongForm, SongSearchForm, UploadArtistForm
...
def home():
    ...
    podcasts = Podcast.query.limit(10).all()
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash("Your account has been created and you can login","success")
...
def is_admin(f):
    ...
    def wrap(*args, **kwargs):
        if current_user.IsAdmin == True:
            ...
            flash('Admin privilege only', 'danger')
            return redirect(url_for('home'))
    return wrap
...
@app.route("/upload/artist/new", methods=['GET', 'POST'])
@login_required
@is_admin
def new_artist():
    form = UploadArtistForm()
    if form.validate_on_submit():
        artist = Artist(name=form.name.data, email=form.email.data)
        db.session.add(artist)
        db.session.commit()
        flash('Artist has been added successfully!', 'success')
        return redirect(url_for('home'))
    return render_template('upload_artist.html', title='New Artist', form=form)
...
def search_song():
    ...
    return render_template('search_song.html', title='search song', songs=songs, form=form)
...
def search_podcast():
    ...
    if form.validate_on_submit():
        ...
        results = query.all()
        return render_template('result.html', title='results', results=results)
    return render_template('search_podcast.html', title='search podcast',podcasts=podcasts, form=form)
